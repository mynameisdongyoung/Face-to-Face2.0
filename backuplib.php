...
function facetoface_backup_mods($bf, $preferences)
{
    ...
    $facetofaces = $DB->get_records('facetoface', 'course', array($preferences->backup_course=>'id'));
//    $facetofaces = $DB->get_records('facetoface', array('course'=>$preferences->backup_course->'id'));
    if ($facetofaces) {
...
function facetoface_backup_session_field($bf, $preferences)
{
    ...
    $sessionfields = $DB->get_records('facetoface_session_field');
    if (!$sessionfields) {
        return $status;
    }
...
function facetoface_backup_one_mod($bf, $preferences, $facetoface)
{
    ...
    if (is_numeric($facetoface)) {
        $facetoface = $DB->get_record('facetoface', array('id'=>$facetoface));
    }
...
function backup_facetoface_sessions($bf, $facetofaceid)
{
    ...
    $sessions = $DB->get_records('facetoface_sessions', 'facetoface', array($facetofaceid=>'id'));
//    $sessions = $DB->get_records('facetoface_sessions', array('facetoface'=>$facetofaceid->'id'));
    if (!$sessions) {
        return $status;
    }
...
function backup_facetoface_sessions_dates($bf, $sessionid)
{
    ...
    $dates = $DB->get_records('facetoface_sessions_dates', 'sessionid', array($sessionid=>'id'));
    //$dates = $DB->get_records('facetoface_sessions_dates', array('sessionid'=>$sessionid->'id'));
...
function backup_facetoface_session_data($bf, $sessionid)
{
    ...
    $data = $DB->get_records_sql("SELECT d.id, f.shortname, d.sessionid, d.data
...
function facetoface_check_backup_mods_instances($instance, $backup_unique_code)
{
    ...
    $info[$instance->id.'1'][1] = $DB->count_records('facetoface_sessions', array('facetoface'=>$instance->id));

    if (!empty($instance->userdata)) {
        $info[$instance->id.'2'][0] = get_string('signups', 'facetoface');
        $info[$instance->id.'2'][1] = $DB->count_records_sql(
            "
                SELECT
                    COUNT(s.id)
                FROM
...
function facetoface_check_backup_mods($course, $user_data=false, $backup_unique_code, $instances=null)
{
    ...
    $info[0][1] = $DB->count_records('facetoface', array('course'=>$course));

    $info[1][0] = get_string('sessions', 'facetoface');
    $info[1][1] = $DB->count_records_sql("SELECT COUNT(*)
                                         FROM {facetoface} f,
