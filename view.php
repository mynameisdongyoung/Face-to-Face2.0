        ...
        foreach($sessions as $session) {
            ...
            if ($editsessions) {
                $options .= ' <a href="sessions.php?s='.$session->id.'" title="'.get_string('editsession', 'facetoface').'">'
                    . '<img src="'.$CFG->wwwroot.'/pix/t/edit.gif" class="iconsmall" alt="'.get_string('edit', 'facetoface').'" /></a> '
                    . '<a href="sessions.php?s='.$session->id.'&amp;c=1" title="'.get_string('copysession', 'facetoface').'">'
                    . '<img src="'.$CFG->wwwroot.'/pix/t/copy.gif" class="iconsmall" alt="'.get_string('copy', 'facetoface').'" /></a> '
...
function get_locations($facetofaceid)
{
    ...
    $locationfieldid = $DB->get_field('facetoface_session_field', 'id,shortname,name',array());
    if (!$locationfieldid) {
        return array();
    }
    ...
    if ($records = $DB->get_records_sql($sql)) {
        ...
        foreach ($records as $record) {
            $locationmenu[$record->name] = $record->name;
            $i++;
        }
